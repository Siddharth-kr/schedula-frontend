          ...
          <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
            ...
            <div className="space-y-8">
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2">
                      <h1 className="text-3xl font-bold tracking-tight text-[var(--ink)] font-serif">{doctor.name}</h1>
                      <span className="inline-flex items-center justify-center gap-1 rounded-full bg-amber-50 px-3 py-1.5 text-sm font-bold text-amber-700 ring-1 ring-inset ring-amber-600/20">
                        ...
                        {doctor.rating} <span className="font-medium text-amber-700/70 ml-1">({doctor.reviewCount} reviews)</span>
                      </span>
                    </div>
                    <p className="text-lg font-semibold text-[var(--brand)] mb-3">{doctor.specialty}</p>
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-6 gap-y-2 text-sm font-medium text-[var(--muted)]">
              ...
              <div className="rounded-3xl border border-[var(--line)] bg-white p-6 sm:p-8 shadow-sm">
                ...
                <h2 className="text-xl font-bold text-[var(--ink)] font-serif mb-6">Professional Information</h2>
                <dl className="grid sm:grid-cols-2 gap-y-6 gap-x-8 text-sm">
                  <div>
                    <dt className="font-semibold text-[var(--muted)] mb-1 uppercase tracking-wider text-xs">Specialization</dt>
                    <dd className="font-medium text-[var(--ink)] text-base">{doctor.specialty}</dd>
                  </div>
                  <div>
                    <dt className="font-semibold text-[var(--muted)] mb-1 uppercase tracking-wider text-xs">Experience</dt>
                    <dd className="font-medium text-[var(--ink)] text-base">{doctor.experienceYears} Years</dd>
                  </div>
                  {doctor.qualification && (
                    <div>
                      <dt className="font-semibold text-[var(--muted)] mb-1 uppercase tracking-wider text-xs">Qualification</dt>
                      <dd className="font-medium text-[var(--ink)] text-base">{doctor.qualification}</dd>
                    </div>
                  )}
                  <div>
                    <dt className="font-semibold text-[var(--muted)] mb-1 uppercase tracking-wider text-xs">Consultation Fee</dt>
                    <dd className="font-medium text-[var(--ink)] text-base">${doctor.consultationFee}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div className="rounded-3xl border border-[var(--line)] bg-white p-6 sm:p-8 shadow-md">
                <div className="mb-6 flex items-end justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-[var(--ink)] font-serif">${doctor.consultationFee}</p>
                  </div>
                </div>
                  ...
                  {previewDates.length > 0 ? (
                    <div className="space-y-4">
                      {previewDates.map(date => (
                        <div key={date}>
                          <p className="text-xs font-bold text-[var(--muted)] mb-2 uppercase tracking-wider">{formatDate(date)}</p>
                          <div className="flex flex-wrap gap-2">
                            {groupedSlots[date].slice(0, 4).map(slot => (
                              <div key={slot.id} className="rounded-md border border-[var(--brand)]/30 bg-[var(--sage)]/10 px-2.5 py-1 text-xs font-semibold text-[var(--brand-deep)]">
                                {slot.startTime}
                              </div>
                            ))}
                            {groupedSlots[date].length > 4 && (
                              <div className="rounded-md border border-[var(--line)] bg-slate-50 px-2.5 py-1 text-xs font-semibold text-[var(--muted)]">
                    ...
                    <div className="rounded-xl border border-dashed border-[var(--line)] bg-slate-50 p-4 text-center">
                      <p className="text-sm font-medium text-[var(--ink)]">No appointments available at the moment.</p>
                      <p className="text-xs text-[var(--muted)] mt-1">Check back later for open slots.</p>
                    </div>
                  )}
                </div>
                ...
                <Link 
                  ...
                  className="flex w-full items-center justify-center rounded-xl bg-[var(--brand)] px-5 py-4 text-base font-bold text-white shadow-sm transition-all hover:bg-[var(--brand-deep)] hover:shadow-md active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-offset-2"
                >
                  Book Appointment
                </Link>
